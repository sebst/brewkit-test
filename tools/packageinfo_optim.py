...
async def get_package_info_dist(pkg_name, dist_api=DIST_PKGX_API_BASE_URL):
    pkg_info = {}
        ...
        for arch in ARCHS:
            ...
            async with session.get(url) as response:
                if response.status == 200:
                    versions = (await response.text()).splitlines()
            ...
            for v in versions:
                ...
                async with session.get(v[3]) as response:
                    if response.status == 200:
                        versions_dict[v[0]]['sha256sum'] = (await response.text()).split()[0]
                    else: print(f"Error: finding sha256sum for {pkg_name} on {arch} @ {v[3]}")
                async with session.head(v[1]) as response:
                    if response.status == 200:
...
async def get_database(pantry=PANTRY_PKGX, dist_api=DIST_PKGX_API_BASE_URL):
    pkg_info = {}
    pkg_names = get_package_names_from_pantry(pantry)
    async def do_it(pkg_name, pantry, dist_api):
        nonlocal pkg_info
        pkg_info[pkg_name] = {}
        pkg_info[pkg_name]['pantry'] = get_package_info_pantry(pkg_name, pantry)
        pkg_info[pkg_name]['dist'] = await get_package_info_dist(pkg_name, dist_api)

    async with asyncio.TaskGroup() as tg:
        for i, pkg_name in enumerate(pkg_names):
            tg.create_task(do_it(pkg_name, pantry, dist_api))
...
async def main():
    ...
    pkg_info = await get_database()
    # pprint(pkg_info)
    with open('_optim_db.json', 'w') as f:
        json.dump(pkg_info, f, indent=4)
    with open('_optim_db-minified.json', 'w') as f:
